...
class TopicModel(torch.nn.Module):
    ...
    def __init__(self, config):
        ...
        self.text_specific1 = torch.nn.Linear(1024*2, 128)
...
class RemoveTopic(TopicModel):
    def __init__(self, config):
        ...
        self.text_specific1 = torch.nn.Linear(1024, 128)
...
def train_test_each(args, tensor_split):
    ...
    for i in range(args.epochs):
        ...
        if f1 > best_dev_performance:
            ...
            torch.save(current_model.state_dict(), './best_models/' + args.model + "_" + args.baseline + '.pth')
    ...
    with open('./best_models/' + args.model + "_" + args.baseline + '.json', "w", encoding="UTF-8") as f:
